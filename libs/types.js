 ...
 *
 * We call them "pseudo-type" because they are not instantiable. They're just
 * convenient Object that take advantage of the mechanics of the `instanceof`
 * operator to allow to type check any value.
 */
...
/**
 * Allow to check if a value is an ECMAScript primitive.
 *
 * As of ES2020, ES primitive are Boolean, Number, String, Symbol and BigInt.
 *
 * Technically, `null` and `undefined` are also primitive values, but due to
 * their nature we assume to special case them through the Nullish pseudo-type.
 *
 * In other words if you want to check for all possible ES primitive,
 * you'll have to opt-in yourself for `null` and `undefined` rather than
...
/**
 * Check if an object is a simple key/value owner.
 *
 ...
 *  - All falsy values
 *  - All iterable (including String, Array, Set, Map, etc.) that won't yield a value
 ...
 *
 * With such a definition, some object will always be considered Empty :
 * Error, Date, RegExp, WeakMap, WeakSet, ArrayBuffer, etc.
 */
export const Empty = {
  [Symbol.hasInstance]: (value) => (
